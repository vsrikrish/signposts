...
def eijgenraam(Xs,                          # list of dike heightenings
    ...
    total_failure = 1 - reduce(operator.mul, [1 - p for p in failure_probability], 1)
...
# are ignored.
model.levers = [RealLever("Xs", 0.0, 500.0, length=6),
...
##------------------------------------------------------------------------------
policies = cache("output", lambda: optimize(model, "BorgMOEA", 100000, epsilons=[1, 5, 0.00001]))
